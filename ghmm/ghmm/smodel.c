...
*                       from $Date: 2015-09-25 20:03:19 -0400 (Fri, 25 Sep 2015) $
...
#define CUR_PROC "ghmm_cmodel_free"
  ...
  for (i = 0; i < (*smo)->N && (*smo)->s; i++) {
    ...
    ighmm_cmatrix_free(&(state->out_a_num), (*smo)->cos);
...
# define CUR_PROC "ghmm_cmodel_copy"
  ...
  for (i = 0; i < smo->N; i++) {
    ...
    sm2->s[i].out_a_num = ighmm_cmatrix_alloc (smo->cos, nachf);
    if (!sm2->s[i].out_a || !sm2->s[i].out_a_num) {
      GHMM_LOG_QUEUED(LCONVERTED);
      goto STOP;
    }
    ...
    for (j = 0; j < nachf; j++) {
        for (k = 0; k < smo->cos; k++){
            ...
            sm2->s[i].out_a_num[k][j] = smo->s[i].out_a_num[k][j];
        }
